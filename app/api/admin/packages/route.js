...
export async function GET() {
  try {
    await connectDB();
    const packages = await TourPackage.find({ active: true }).sort({ createdAt: -1 }).lean();
...
export async function POST(request) {
  ...
  try {
    ...
    const { name, destination, duration, price, description, image, category, featured } = body;
    if (!name || !destination || !duration || price === undefined) {
      return NextResponse.json({ error: 'Name, destination, duration and price are required.' }, { status: 400 });
    }

    await connectDB();
    const item = await TourPackage.create({ name, destination, duration, price, description, image, category, featured });
